...
3. **Individual Tracking**: done with the Hungarian Method for the assignment problem, this takes the CSV from the above and reconstructs the path an individual takes, as well as inferring speed, size, and direction using micrometer units. (NOT STARTED)
...
- `matplotlib`, `PIL`, `requests`, `cv2`, and `shutil` for working with images
- `os` for listing files in directories
